...
def breadth_first_tree_search(problem):
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            return node, max_frontier_size, nodes_visited
        frontier.extend(node.expand(problem))
        max_frontier_size = max(max_frontier_size, len(frontier))
    return None, max_frontier_size, nodes_visited
        ...
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
                if problem.goal_test(child.state):
                    # for BFS the space complexity includes the length of explored nodes
                    return child, max_frontier_size + len(explored), nodes_visited
                frontier.append(child)
                max_frontier_size = max(max_frontier_size, len(frontier))
    return None, max_frontier_size + len(explored), nodes_visited
...
def best_first_graph_search(problem, f, display=False):
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            ...
            return node, max_frontier_size + len(explored), nodes_visited
        explored.add(node.state)
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
                frontier.append(child)
